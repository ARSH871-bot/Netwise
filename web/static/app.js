...
/**
 * A plain anchor pointing at an evidence block's id (#236). `href` is set
 * as a property rather than through `setAttribute`, matching how the rest
 * of this file treats DOM nodes as plain objects -- and it is what a real
 * `<a>` element's `.href` already is: a reflected attribute, not a method
 * call. Deliberately not a click handler: a native link needs no JavaScript
 * to work, degrades to normal browser behaviour (open in a new tab, copy
 * the link, etc.), and is exercised by a test the same way any other piece
 * of rendered structure is -- by reading what got built, not by simulating
 * a click nothing in this project's test harnesses can fire.
 */
function renderEvidenceLink(evidenceId) {
  const link = el("a", "evidence-link", "View the evidence this came from");
  link.href = `#${evidenceId}`;
  return link;
}
 ...
 *
 * `evidenceId` is a DOM id for the evidence block, generated by the caller
 * (see `renderFindings()`) from a counter, never from `finding.id`. The
 * comment above this function's neighbours already establishes that
 * `finding.id` is not unique -- RT-000 can name two different findings in
 * the same render. An href built from it would then point `#RT-000` at
 * whichever evidence block happened to render last, silently taking the
 * reader to the wrong finding's evidence (#236).
 */
function renderFinding(finding, variant, icon, badgeText, evidenceId) {
  ...
  //
  // Absent `explanation` used to render nothing at all -- not an empty
  // labelled box promising prose that does not exist, which is what the
  // pre-#31 placeholder did. #236 changes that for status="found" findings
  // specifically: silence there does not read as "nothing to explain", it
  // reads as "nothing was missed", and the second claim is not one this
  // finding can back. So the found-but-unexplained case now says so.
  //
  // PROVENANCE (#236, phase A). Every explanation -- model or fallback -- is
  // a paraphrase of `evidence.detail`/`evidence.source` directly above it in
  // the same card. That link was true but invisible: nothing on screen said
  // where the sentence came from, or let a reader jump to the raw line it
  // was grounded in. `renderEvidenceLink()` is that jump, built as a plain
  ...
  if (finding.status === "found") {
    if (finding.explanation) {
      ...
      // A missing evidenceId must never become a link to "#undefined" --
      // reviewed on this PR (Arsh + Samika, #255): that is a broken trail
      // dressed as a working one, worse than the plain prose it replaces. A
      // caller that forgets to pass one gets an honest card with no link,
      // not a card that looks wired and is not.
      if (evidenceId) {
        explanation.appendChild(renderEvidenceLink(evidenceId));
      }
      card.appendChild(explanation);
      // The local-model boundary (#281): when a non-loopback OLLAMA_HOST was
      // refused, the fallback text above is the deterministic path taken for
      // that reason specifically, and this notice says so without ever
      // showing the refused host itself.
      ...
      card.appendChild(
        el(
          "p",
          "no-explanation",
          "No explanation was generated for this finding. The evidence " +
...
function renderFindingSections(findings) {
  ...
  // One counter for the whole render, not per section or per finding.
  // `finding.id` cannot be used for this -- the comment above `el()`'s
  // neighbours already establishes it collides (RT-000 twice in one render)
  // -- so evidence blocks get their own id from a source that genuinely
  // never repeats within a page load (#236).
  let evidenceCounter = 0;
  const nextEvidenceId = () => `evidence-${++evidenceCounter}`;
  ...
  const sections = [
    renderSection(
      ...
      blind,
      (f) => renderFinding(f, "blind", "⚠", "could not check", nextEvidenceId())
    ),
    renderSection(
      "Problems found",
      "Issues the analysis identified, most serious first.",
      problems,
      (f) => renderFinding(f, f.severity, "●", f.severity, nextEvidenceId())
    ),
    renderSection(
      "Checked — nothing found",

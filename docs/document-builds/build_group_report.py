...
action("student IDs",
       "Shubham's and Samika's student IDs are still missing, and Ankeet's "
       "was taken from his cover sheet on an earlier report rather than from "
       "him. Until all four are confirmed the cover lists names only: a "
       "table naming two of four with IDs and saying nothing about it is "
       "worse than one that plainly has none. Add them to docs/identity.json "
       "and all four appear.")
...
figure("architecture-layers.png",
       "Figure 1. The three layers the product is built from, and what runs in "
       "each. Layer 1 is Batfish, in a container we did not write. Layer 2 is a "
       "locally hosted model, also not ours. Layer 3 is the web interface, which "
       "we wrote entirely. Generated by tools/make_diagrams.py and committed to "
       "the repository.")
figure("ownership-boundary.png",
       "Figure 2. The same distinction from the other direction: grey is "
       "somebody else's work, blue is ours. Generated by tools/make_diagrams.py "
...
figure("process-flow.png",
       "Figure 3. What happens when a user presses Scan Now, step by step. The "
       "two amber steps are the ones that can stop and say so, and they are "
       "why a result on screen can be trusted.", width=4.4)
...
h("How risk is classified", 3)
p("Each check gives a finding its severity when it first creates it. After "
  "every check has run, a separate step called risk scoring looks at the "
  "whole list together and can raise or lower that severity. No single check "
  "can do this on its own, because no check can see what the others found. "
  "Risk scoring is only allowed to do two things: change the severity of a "
  "found problem, and put the list in order. Nothing else. It can never touch "
  "a finding that says a check could not run, and it can never add or remove "
  "a finding. Hiding a blind spot by quietly changing its rating would be "
  "exactly the mistake this whole project is built to avoid.")
p("The rules are checked in order. The first one that matches decides the "
  "severity:")
table(["Rule", "When it applies", "Severity", "Why"], [
    ["A dead rule",
     "The finding is about an access-list line that can never take effect, "
     "because an earlier line already caught the traffic", "Low",
     "A rule that can never fire cannot let anything through. It is still "
     "worth reporting, because someone may think it is protecting them when "
     "it is not, but it is a tidy-up problem, not a real opening"],
    ["A blanket permit",
     "The finding is itself about a rule that allows any source to reach any "
     "destination", "High",
     "This is the mistake that most often causes a real breach. It usually "
     "gets added to fix a complaint and is never removed, and it quietly "
     "cancels out every rule written after it"],
    ["A reachability failure", "The finding comes from the routing check",
     "Medium",
     "The routing check calls a broken path high severity, and that is fair "
     "on its own terms: a network that cannot reach itself is broken. But "
     "this is a security tool. An outage that everyone notices within minutes "
     "should not rank above a door that is silently left open. This is the "
     "only rule that lowers a check's own rating instead of raising it"],
    ["Anything else", "No rule above matches", "Whatever the check set",
     "This is the normal case. A set of rules that overrides everything is "
     "not prioritising anything, it is just hiding the judgement somewhere "
     "harder to see"],
], widths=[1.1, 1.7, 0.8, 2.7])
p("One case needs explaining, because two different findings can quote the "
  "same line of text. A finding that is about a blanket permit rule is rated "
  "high. A different finding that only mentions the same rule, as the reason "
  "something else never fires, is not rated high for that. Checking the "
  "dead-rule case first is what keeps the two apart.")

h("How compliance checking works", 3)
p("Policy compliance asks a narrower question than the other checks. The "
  "other checks ask what a configuration can do. This one asks whether it "
  "does what somebody wrote down that it should do. Every rule is one of two "
  "kinds. A prohibition says some traffic must be blocked. A requirement says "
  "some traffic must be allowed. These two kinds are tested in opposite ways, "
  "because testing them the same way gives the wrong answer.")
table(["Rule kind", "Says", "How it is tested", "An empty result means"], [
    ["Prohibition", "This traffic must be blocked",
     "Search the whole space of that traffic for anything the configuration "
     "permits", "The policy holds"],
    ["Requirement", "This traffic must be allowed",
     "Search the whole space of that traffic for anything the configuration "
     "denies", "The policy holds"],
], widths=[0.9, 1.6, 2.7, 1.1])
p("Both searches use a Batfish feature called searchFilters, the same one "
  "described in the table in section 4.1. It proves something is true for "
  "every possible flow of traffic, not just one example. We tried the more "
  "obvious way first: asking whether the traffic can reach its destination. "
  "We tested it and found it gives the wrong answer here. If a configuration "
  "has no route to a destination, it reports that destination as unreachable "
  "no matter what the firewall rules say, so a wide-open, unsafe access list "
  "can still come back looking like a pass. Testing the filter rules directly "
  "does not depend on whether a route exists. It only depends on what the "
  "rules say.")
p("The example rules we tested say five things about the reference network. "
  "The internal network may only reach two named servers. One of those "
  "servers must accept nothing but encrypted web traffic. No address claiming "
  "to come from outside the internal network may enter it. And two specific, "
  "legitimate flows must keep working: name lookups to the approved server, "
  "and encrypted web traffic to the internal server. Those last two rules "
  "exist for a reason. A check that only ever looks for rules that are too "
  "open is only doing half its job. A rule that is tightened until it breaks "
  "the network it protects is also a real failure, and we tested a "
  "configuration that fails in exactly that way, to prove the check catches "
  "that too.")
p("A user can give the check a written policy of their own, instead of the "
  "example rules above. Every finding says plainly which of the two was used. "
  "This is the least finished part of the project: policy compliance reads a "
  "user's own policy, but the other two checks still do not, and section 10.3 "
...
bullets([
    ...
    "that fails if a rule points at a path that does not exist. "
...
# ================================================================ 12
h("12  References", 1)
p("Each entry notes what it was used for, because a reference "
...
code("python -m tools.preflight                            # check the machine\n"
     ...
     "python -m tools.make_diagrams                        # Figures 1, 2 and 3\n"

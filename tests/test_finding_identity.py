...
def test_fixing_one_of_two_identical_looking_dead_rules_is_visible():
    ...
    assert result["resolved"] == [MESSY_AC_002]
    assert result["unchanged"] == [MESSY_AC_003]
    assert result["new"] == []


def test_counts_are_honoured_not_collapsed():
    """Two copies before, one after: one unchanged, one resolved -- not zero, not two."""
    result = identity.compare([INSECURE_AC_001, INSECURE_AC_001], [INSECURE_AC_001])
    assert len(result["unchanged"]) == 1
    assert len(result["resolved"]) == 1


def test_a_new_problem_and_a_resolved_one_are_separate():
    result = identity.compare([INSECURE_AC_001], [MESSY_AC_001])
    assert result["resolved"] == [INSECURE_AC_001]
    assert result["new"] == [MESSY_AC_001]
...
def test_a_real_rewording_is_flagged_as_possibly_the_same():
    ...
    Identity changes -- the text changed -- so it is resolved + new. It is
    ...
    assert result["resolved"] == [REWORDED_BEFORE]
    assert result["new"] == [REWORDED_AFTER]
    assert len(result["possibly_same"]) == 1


def test_a_flag_never_removes_anything_from_new_or_resolved():
    ...
    assert result["resolved"] == [INSECURE_AC_002]
    assert result["new"] == [INSECURE_AC_001]
...
def test_a_line_shift_is_flagged_not_merged():
    ...
    undefined-reference finding's source from `[37]` to `[36]`:
        ...
        unchanged 6   resolved 1 ([37])   new 1 ([36])
        possibly_same 1   similarity 1.0

    The same problem therefore reads as one resolved and one new. That is
    ...
    assert result["resolved"] == [MESSY_AC_004]
    assert result["new"] == [shifted]
    (old, fresh, score), = result["possibly_same"]
    assert score == 1.0
